...
export default function ({ data: rawData = [], meta = {}, t }) {
  ...
  const data = useMemo(() => {
    ...
    result.forEach((row) => {
      ...
      row.balance_sign =
        currentRunningBalance === 0 ? undefined : currentRunningBalance > 0 ? "DR" : "CR";
    });
  ...
  const columns = [
    ...
    },
    {
      ...
      ids: [
        ...
        "accountCounterpart_full_description",
      ],
      style: { maxWidth: "15em" },
      ...
      cell: ({ row }) =>
        [
          ...
          showColumn("accountCounterpart_full_description")
    ...
    },
    {
      ...
      cell: ({ value }) => utils.formatNumber(value),
      footerValue: ({ data }) =>
        data.reduce((red, row) => utils.round(red + (row.sign === "DR" ? row.count_dr : 0), 2), 0),
    ...
    },
    {
      ...
      cell: ({ value }) => utils.formatNumber(value),
      footerValue: ({ data }) =>
        data.reduce((red, row) => utils.round(red + (row.sign === "CR" ? row.count_cr : 0), 2), 0),
      footer: ({ value }) => utils.formatNumber(value),
    },
    {
      id: "sum_value_dr",
      ...
      headerClassName: "number",
      className: ({ row, value }) =>
        `number ${row ? getColor(nature, row?.sign, value) : nature === "DR" ? "positive" : "negative"}`,
      cell: ({ row, value }) => (row.sign === "DR" ? utils.formatCurrency(value) : undefined),
      footerValue: ({ data }) =>
        data.reduce(
          (red, row) => utils.round(red + (row.sign === "DR" ? row.sum_value_dr : 0), 2),
          0,
        ),
    ...
    },
    {
      ...
      headerClassName: "number",
      className: ({ row, value }) =>
        `number ${row ? getColor(nature, row?.sign, value) : nature === "CR" ? "positive" : "negative"}`,
      cell: ({ row, value }) => (row.sign === "CR" ? utils.formatCurrency(value) : undefined),
      footerValue: ({ data }) =>
        data.reduce(
          (red, row) => utils.round(red + (row.sign === "CR" ? row.sum_value_cr : 0), 2),
          0,
        ),
      footer: ({ value }) => utils.formatCurrency(value),
    },
    {
      ...
      cell: ({ row, value }) =>
        `${utils.formatCurrency(Math.abs(value))}${row.balance_sign ? "\u00A0" + t(`/financial/accounting/sign/enum/${row.balance_sign}/sign`) : ""}`,
    },
    {
      id: "tags",
      header: t("/@word/tags"),
      ...
      <div className={`report-container ${settings?.pageSize ?? "a4"} ${settings?.orientation}`}>
        ...
        <main>
          <div className="content">
            <Table columns={columns} visibleColumns={showColumns} data={data} />
          </div>
        </main>
      </div>
    </div>
